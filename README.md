...
## Новое и исправленное в 0.3.9

- Одна строка вкладок с отдельной прокруткой слева/справа.
- Прямое подключение перенесено в меню `Подключение` (`Ctrl+L`).
- Новый компактный и логичный интерфейс вкладок `Обзор`, `Утилиты`, `Массовые действия`, `Wake-on-LAN`.
- Стабилизированы размеры кнопок, иконок, инпутов и отступы панелей.
- Фильтр `Только активные` расположен под списком ПК домена.
...
Версия берётся из `tag_name` последнего опубликованного GitHub Release (`v0.3.9`, `0.3.9` и т. п.) и сравнивается с версией текущей сборки.
...
- Реестр: дерево ключей + таблица значений.
- Файловый менеджер: двухпанельный режим с деревьями каталогов.
- Журнал Windows: двойной клик открывает подробности события.
- Новая непрерывная строка вкладок с прокруткой ◀ / ▶.
- Унифицированы отступы иконок на кнопках.
- Утилиты сгруппированы в компактные панели.
